assign.par.stoich <- function(par,y.names,Pref.default=1,pref.use=FALSE,CPOM.dif=FALSE)
  ...
  masses <- get.inpind.parval.taxaprop(par.names = c("M"),        
  ...
  # das geht wohl noch nicht f?r zeitabh. Inputs
  par.feedtype <- get.inpind.parval.taxaprop.traits(trait.names="feedingtype",
                                                    y.names=y.names,
                                                    par=par,inp=inp)$feedingtype$parvals
  if(length(grep("food", names(par))) > 0)
  {
    par.foodsources <- get.inpind.parval.taxaprop.traits(trait.names="food",
                                                         y.names=y.names,
                                                         par=par)$food$parvals
  } else {
    par.foodsources <- NA
  }
  ...
  for (i in 1:length(names.inverts) )
  {
    ...
    if ( sum(!is.na(ftypes))>0 )
    {
      ...
      if ( sum(which(scra %in% ftypes)) > 0 )  
      {
        if(length(na.omit(par.foodsources)) <= 0)     # general case, no information on food sources (consider that they feed on all types of algae)
        {
          for (j in 1:length(names.algae))
          {
            ...
            temp           <- numeric(0)
            temp[1]        <- par["Cons_Invertebrates_Scra_Scra"]
            names(temp)[1] <- paste("Cons",invert,alga,invert,sep="_")
            temp[2]        <- par["Cons_Invertebrates_Scra_Algae"]
            names(temp)[2] <- paste("Cons",invert,alga,alga,sep="_")
            temp[3]        <- par["Cons_Invertebrates_Scra_FPOM"] 
      ...
      if ( sum(which(shred %in% ftypes)) > 0 )  
      {
        if(length(na.omit(par.foodsources)) <= 0)     # general case, no information on food sources (consider that they feed on not determined CPOM)
        {
          ...
          ind2 <- ind2+1
          par.stoich.cons[ind2] <-  par["Cons_Invertebrates_Shred_Shred"]  
          names(par.stoich.cons)[ind2] <- paste("Cons",invert,"CPOM",invert,sep="_")
          
          ind2 <- ind2+1
          par.stoich.cons[ind2] <- par["Cons_Invertebrates_Shred_CPOM"]   
          names(par.stoich.cons)[ind2] <- paste("Cons",invert,"CPOM","CPOM",sep="_")
          
          ind2 <- ind2+1
          par.stoich.cons[ind2] <- par["Cons_Invertebrates_Shred_FPOM"]   
          names(par.stoich.cons)[ind2] <- paste("Cons",invert,"CPOM","FPOM",sep="_")
        } else {                                 # Specific case where
          # information on food sources
          # is available
          
          # shredders feeding on living macrophytes, for the moment the only 'macrophytes' are filamentous algae
          if ( sum(which(macpl %in% fsources)) > 0 ) 
          {                                    
            food   <- "filamentousAlgae"       
            shred2 <- "Shredm"                 
            ...
            temp           <- numeric(0)
            temp[1]        <- par[paste("Cons_Invertebrates", shred2, shred2, sep = "_")]  
            names(temp)[1] <- paste("Cons",invert,food,invert,sep="_")
            temp[2]        <- par[paste("Cons_Invertebrates", shred2, food, sep="_")]   
            names(temp)[2] <- paste("Cons",invert,food,food,sep="_")
            temp[3]        <- par[paste("Cons_Invertebrates", shred2, "FPOM", sep="_")] 
          ...
          # shredders feeding on dead plants
          if ( sum(which(dpl %in% fsources)) > 0 ) 
          {                                   
            food <- "CPOMp"                     
            shred2 <- "Shredp"                   
          ...
          # shredders feeding on dead animals
          if ( sum(which(dinv %in% fsources)) > 0 ) 
          {                                   
            food <- "CPOMa"                     
            shred2 <- "Shreda"                   
      ...
      if ( sum(which(filt %in% ftypes)) > 0 )  
      {
        if(length(na.omit(par.foodsources)) <= 0)
        {
          ...
          ind2 <- ind2+1
          par.stoich.cons[ind2] <-  par["Cons_Invertebrates_Filt_Filt"]  
          names(par.stoich.cons)[ind2] <- paste("Cons",invert,"SusPOM",invert,sep="_")
          
          
          #ind2 <- ind2+1
          #par.stoich.cons[ind2] <- par["Cons_Invertebrates_Filt_SusPOM"]   
          #names(par.stoich.cons)[ind2] <- paste("Cons",invert,"SusPOM","SusPOM",sep="_")
          
          ind2 <- ind2+1
          par.stoich.cons[ind2] <- par["Cons_Invertebrates_Filt_FPOM"]   
      ...
      if ( sum(which(coll %in% ftypes)) > 0 )  
      {
        ...
        ind2 <- ind2+1
        par.stoich.cons[ind2] <-  par["Cons_Invertebrates_Coll_Coll"]  
        names(par.stoich.cons)[ind2] <- paste("Cons",invert,"FPOM",invert,sep="_")
        
        ind2 <- ind2+1
        par.stoich.cons[ind2] <- par["Cons_Invertebrates_Coll_FPOM"]   
      ...
      {
        ## General case, no information on prey types
        ## can feed on any prey with smaller weight
        ...
        if(length(na.omit(par.foodsources)) <= 0)
        {
          ...
          for ( j in 1:length(prey))
          {
            if (mass[i] > ratio_pred_prey*mass_prey[j])
            {
              ...
              temp           <- numeric(0)
              temp[1]        <- par["Cons_Invertebrates_Pred_Pred"]  
              names(temp)[1] <- paste("Cons",invert,prey[j],invert,sep="_")
              temp[2]        <- par["Cons_Invertebrates_Pred_Prey"]   
              names(temp)[2] <- paste("Cons",invert,prey[j],prey[j],sep="_")
              temp[3]        <- par["Cons_Invertebrates_Pred_FPOM"]   
          ...
          ## Information on prey types, since no information on microinvertebrates and vertebrates is available, only consider as predators the taxa potenitally able to prey macroinvertebrates
          if(sum(which(macinv %in% fsources)) > 0)
          {
            prey <- names.inverts[-i]
            mass_prey <- mass[prey]
      ...
      if ( sum(which(pierc %in% ftypes)) > 0 )  
      {
        if(length(na.omit(par.foodsources)) <= 0)     # general case, no information on food sources (consider that they feed on both macrophytes and other macroinvertebrate)
        {
          ...
          for ( j in 1:length(prey))
          {
            ...
            temp           <- numeric(0)
            temp[1]        <- par[paste("Cons_Invertebrates", pierc2, pierc2, sep="_")]  
            names(temp)[1] <- paste("Cons",invert,prey[j],invert,sep="_")
            temp[2]        <- par[paste("Cons_Invertebrates", pierc2, "Prey", sep="_")]   
            names(temp)[2] <- paste("Cons",invert,prey[j],prey[j],sep="_")
            temp[3]        <- par[paste("Cons_Invertebrates", pierc2, "FPOM", sep="_")]   
            names(temp)[3] <- paste("Cons",invert,prey[j],"FPOM",sep="_")
          ...
          temp[1]        <- par[paste("Cons_Invertebrates", pierc2, pierc2, sep = "_")]    
          names(temp)[1] <- paste("Cons",invert,food,invert,sep="_")
          temp[2]        <- par[paste("Cons_Invertebrates", pierc2, food, sep="_")]      
          names(temp)[2] <- paste("Cons",invert,food,food,sep="_")
          temp[3]        <- par[paste("Cons_Invertebrates", pierc2, "FPOM", sep="_")]     
          names(temp)[3] <- paste("Cons",invert,food,"FPOM",sep="_")
    ...
    if(CPOM.dif == FALSE)
    {
      par.stoich.cons[ind2] <- par["Death1_Invertebrates_Invertebrates"] 
    } else {
      par.stoich.cons[ind2] <- par["Death2_Invertebrates_Invertebrates"] 
    }
    ...
    if(CPOM.dif == FALSE)
    {
      par.stoich.cons[ind2] <- par["Death1_Invertebrates_FPOM"]  
      names(par.stoich.cons)[ind2] <- paste("Death",invert,"FPOM",sep="_")  
    } else {
      par.stoich.cons[ind2] <- par["Death2_Invertebrates_CPOMa"]  
  ...
  for (k in 1:length(names.algae))
  {
    ...
    ind2 <- ind2+1
    par.stoich.cons[ind2] <- par["Death_Algae_Algae"]   
    names(par.stoich.cons)[ind2] <- paste("Death",alga,alga,sep="_")
    
    ind2 <- ind2+1
    par.stoich.cons[ind2] <- par["Death_Algae_FPOM"]   
