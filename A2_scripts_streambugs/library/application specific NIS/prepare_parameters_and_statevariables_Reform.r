...
Invertebrates     <- c(t(Invertebrates))[1:20]  # [1:10]for testing to speed up
...
file.db.feeding    = "databases/db_freshwaterecology_20131206_AP.dat" #XXX
file.db.microhab   = "databases/db_traits_tachet_AP.dat"              # AP
file.db.current    = "databases/db_traits_tachet_AP.dat"              # classic
file.db.temp       = "databases/db_traits_tachet_AP.dat"              # classic
file.db.sapro      = "databases/db_freshwaterecology_20131206_AP.dat"
...
par.invtraits <- construct.invpars.traits(Invertebrates,
                                          ...
                                          file.db.sapro=file.db.sapro,
                                          file.db.spear=file.db.spear,
                                          file.db.bodymass="databases/db_bodymass_meas_aquaplus.dat")  # XXX check which one to use)
...
par <- sysanal.read.distdef("input/parameter_input_unc_Reform.dat")
#par <- sysanal.read.distdef("input/parameter_input_fix.dat") # currently does not work without uncertain parameters
...
if(length(prior.par.defs)>0) prior.par.defs <- transform.priors(prior.par.defs)


# generate par samp to get a parameter sample at the mean of the marginals

parx <- generate.par.samp.matrix(n.samp=1,par.unc=par.unc,verbose=TRUE)
