simulate.streambugs.return.likeli.catchments <- function(par,y.names=y.names,C=TRUE,verbose=TRUE,
                                                         ...
                                                         M.taxa=M.taxa,
                                                         return.res=FALSE,p.obs,p.abs,D.drift,
                                                         catchments,Invertebrates.list)
{
        ...
        round(Death_Invertebrates_O2,digits=10) < 0 )
  {
    ...
    for ( i in 1:length(y.names$reaches))
    {
      ...
      par.stoich.taxa <- assign.par.stoich(par.invtraits,par.stoich.out,y.names.i,CPOM.dif=CPOM.dif)
...
#         reslikeli$loglikeli <- c(reslikeli$loglikeli,reslikeli.i$loglikeli) 
...
#     names(reslikeli$loglikeli) <- y.names$reaches
