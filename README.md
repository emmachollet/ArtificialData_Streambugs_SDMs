...
**Purpose**: This repository provides reproducible data, scripts and plots of the manuscript *["Noise, bias and data limitations: Virtual benchmarking of species distribution models’ robustness for prediction and inference"](future_link)*. The study explores the effects of data limitations on the performance and interpretability of species distribution models (SDMs) using an artificial dataset generated by the mechanistic model *Streambugs*. The study examines how reducing dataset size, missing predictors, and noise in the data affect four SDMs: Generalized Linear Model (GLM), Generalized Additive Model (GAM), Random Forest (RF), and Artificial Neural Network (ANN). 
...
To reproduce the complete workflow on R 4.4.1:

1. Clone this repository.
2. Open `set_folders_structure.r`, set the working directory and run the script.
3. Open `main_streambugs.r` in A2_scripts_streambugs, set the working directory  and run the script.
4. Open `main_sdms.r` in B1_scripts_sdms, set the working directory  and run the script.
5. Generated outputs are stored in folders A3_outputs_streambugs and B2_outputs_sdms.
...
- The environmental data consists of environmental factors selected and calculated based on previous studies. They are derived from the Swiss Federal Office for the Environment (water quality monitoring data, hydrological data, hydromorphological data, land use data), the Swiss Federal Office of Topography (topographical data), and the Swiss Federal Statistical Office (population statistics). They are calculated for monitoring and synthetic sites with the support of a GIS expert.
...
*Figure: Catchment delimitation used to define initial taxa pools. Ask GIS expert to include this as a column when preparing input environmental data.*
...
| filename                      | description                                                  |
| :---------------------------- | ------------------------------------------------------------ |
| **main_streambugs.r**         | **Open. Set working directory here. <br />- Check carefully options at the beginning of the script: taxa selection, catchment selection, nb of sites, ICE simulation or not. <br />- Run to simulate data with Streambugs, and to produce according datasets and plots.** |
